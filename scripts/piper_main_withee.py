...
def main():
    rospy.init_node("pi05_zeno_main_withee")
    ...
    while not rospy.is_shutdown():
        # Wait until all sensors have data (including end effector poses)
        ...
        if not data_ready_logged:
            rospy.loginfo("✓ Successfully receiving observations from robot arms (cameras + joint states + end effector poses)")
        ...
        if action_buffer is None or action_index >= len(action_buffer) - replan_threshold:
            # Keys: observation/image, observation/wrist_image, observation/right_wrist_image,
